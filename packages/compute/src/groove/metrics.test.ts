...
describe('isInGroove', () => {
  ...
  it('upwind: requires angle band AND speed', () => {
    ...
    expect(isInGroove({ pointOfSail: 'upwind', twaAbsRad: 50 * DEG, targetTwaRad: 42 * DEG, bspMs: 6, targetSpeedMs: 6, ...s })).toBe(false);
    expect(isInGroove({ pointOfSail: 'upwind', twaAbsRad: 44 * DEG, targetTwaRad: 42 * DEG, bspMs: 5, targetSpeedMs: 6, ...s })).toBe(false);
  });
