...
export type ValidateResult = { ok: true; layout: MastLayout } | { ok: false; errors: string[] };
...
function validateTile(
  ...
  if (typeof tile.label !== 'string' || tile.label.length === 0)
    errors.push(`${where}: label must be a non-empty string`);
  if (typeof tile.units !== 'string' || !DISPLAY_UNITS.has(tile.units))
    errors.push(`${where}: invalid units "${String(tile.units)}"`);
  if (
    typeof tile.decimals !== 'number' ||
    !Number.isInteger(tile.decimals) ||
    tile.decimals < 0 ||
    tile.decimals > 3
  ) {
  ...
  if (tile.thresholds !== undefined) {
    if (!Array.isArray(tile.thresholds)) errors.push(`${where}: thresholds must be an array`);
      ...
      tile.thresholds.forEach((th, i) => {
        if (
          !th ||
          typeof th !== 'object' ||
          !THRESHOLD_COLORS.has((th as { color?: unknown }).color as string)
        ) {
  ...
  layout.pages.forEach((p, pi) => {
    ...
    if (!Array.isArray(p.tiles) || p.tiles.length === 0) {
      ...
      if (isGridKind(p.grid) && p.tiles.length > GRID_CAPACITY[p.grid]) {
        errors.push(
          `${where}: ${p.tiles.length} tiles exceed grid "${p.grid}" capacity ${GRID_CAPACITY[p.grid]}`,
        );
      }
